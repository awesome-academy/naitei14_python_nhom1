...
class PitchTimeSlot(models.Model):
    ...
    def get_price(self):
        """Tính giá dựa base_price_per_hour của
        pitch, duration của time_slot"""
        return self.pitch.base_price_per_hour * self.time_slot.duration_hours()

    def is_available_on_date(self, booking_date):
        """
        Kiểm tra xem PitchTimeSlot này có còn trống vào ngày
        cụ thể không
        QUAN TRỌNG: time_slot trong Booking là ForeignKey
        trỏ đến PitchTimeSlot (self),không phải TimeSlot
        """
