...
class CustomUserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        ('Kích hoạt tài khoản', {
            'fields': constants.READONLY_ACTIVATION_FIELDS
        }),

        ('Ngày giờ', {
            'fields': (
                'last_login',
            ) + constants.READONLY_TIMESTAMP_FIELDS
        }),
    )
    ...
    def delete_model(self, request, obj):
        if obj.bookings.filter(
            status__in=[
                ...
                BookingStatus.CONFIRMED]).exists():
            messages.error(
                request,
    ...
    def delete_queryset(self, request, queryset):
        blocked = queryset.filter(
            bookings__status__in=[
                BookingStatus.PENDING,
                BookingStatus.CONFIRMED]).distinct()
        if blocked.exists():
            messages.error(
                request,
...
class PitchTimeSlotAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'pitch', 'time_slot', 'pitch__facility')
...
class BookingAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'user', 'pitch', 'time_slot__time_slot', 'voucher')
